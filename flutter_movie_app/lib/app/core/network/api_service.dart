...
class ApiService {
  ...
  Future<TrailerMovieResponse> getVideos(int movieId) async {
    final path = ApiConstants.getTrailers.replaceFirst(":id", "$movieId");
    final response = await _apiClient.get(path: path);
    return TrailerMovieResponse.fromJson(json.decode(response));
  }
}